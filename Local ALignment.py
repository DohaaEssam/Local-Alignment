...
blosum62 = {
    ...
    ('B', 'F'): -3, ('F', 'L'): 0, ('X', 'Q'): -1, ('B', 'B'): 4, ('C', 'V'): 4
}


def display(seq1, seq2):
    while (True):
        print("1.DNA")
        print("2.Protein")
        print("3.Exit")
...
row = len(l1)
col = len(l2)
A = [[0 for x in range(row + 1)] for y in range(col + 1)]


def ProteinAlignment(seq1, seq2):
    row = len(seq1)
    col = len(seq2)
    A = [[0 for x in range(row + 1)] for y in range(col + 1)]
    ...
    if row < col:
        for i in range(1, row + 1, 1):
            if (seq1[i - 1] == seq2[i - 1]) or (seq1[i - 1] != seq2[i - 1]):
                score = blosum62[(seq1[i - 1], seq2[i - 1])]
                totalScore += score
    elif col < row:
